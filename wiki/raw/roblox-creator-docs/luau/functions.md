...
title: Functions
type: raw-source
source_url: https://create.roblox.com/docs/luau/functions
source_type: official-roblox-docs
captured_at: 2026-04-16
captured_by: research-agent-2
category: luau-language
tags: [luau, functions, methods, callbacks, anonymous, variadic, closures]
---
...
> **Info:** The statement `table:Method()` is functionally identical to `table.Method(table)`, and when defining a method, `function table:Method()` is identical to `function table.Method(self)`. This behavior extends to `Instance` methods, since they are also derived from tables.
...
Functions can be passed into other functions, for example, an [anonymous](#anonymous-functions) function can be used to implement a callback that `table.sort()` then uses to sort a list of `Players` from `Players:GetPlayers()`.
...
To set a callback, assign a function to it. For example, `BindableFunction.OnInvoke` is a callback of `BindableFunction`. You can set a named or [anonymous](#anonymous-functions) function to it, and you can call (**invoke**) that function by calling the `:Invoke()` method on the callback. The arguments you pass to `:Invoke()` forward to the callback, and the return value from the callback function returns to the caller of `:Invoke()`.
...
You can assign a function, known as an **event handler**, to execute when an event fires. For example, you can create a function called `onPlayerAdded()` to the `Players.PlayerAdded` event to print the name of whatever player joins. For more information, see [Events](../scripting/events/index.md).
...
Original URL: https://create.roblox.com/docs/luau/functions
GitHub source: https://github.com/Roblox/creator-docs/blob/main/content/en-us/luau/functions.md
Captured: 2026-04-16
