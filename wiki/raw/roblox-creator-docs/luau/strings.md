...
title: Strings
type: raw-source
source_url: https://create.roblox.com/docs/luau/strings
source_type: official-roblox-docs
captured_at: 2026-04-16
captured_by: research-agent-2
category: luau-language
tags: [luau, strings, data-types, string-interpolation, patterns]
---
...
You can use direct matches in a Luau function like `string.match()`, except for [magic characters](#magic-characters). For example, these commands look for the word **Roblox** within a string:
...
| `%c` | A special [control character](https://wikipedia.org/wiki/Control_character) | |
...
Sets also support **ranges** which let you find an entire range of matches between a starting and ending character. This is an advanced feature which is outlined in more detail on the [Lua 5.1 Manual](https://www.lua.org/manual/5.1/manual.html#5.4.1).
...
The `?` quantifier that follows both of the `%s` classes is a safe addition because it makes the space on either side of the `=` sign optional. That means the match succeeds if one (or both) spaces are missing around the equal sign.

In the `string.gsub()` function, you can include capture groups in the replacement string using `%1`, `%2`, `%3`, etc., up to `%9`. Even though Luau supports up to 32 capture groups before throwing an error, you can only reference the first nine with this syntax:
...
Original URL: https://create.roblox.com/docs/luau/strings
GitHub source: https://github.com/Roblox/creator-docs/blob/main/content/en-us/luau/strings.md
Captured: 2026-04-16
